...
describe("ProviderRuntimeEvent", () => {
  ...
  it("decodes command output carrying a pi.rpc.control raw source", () => {
    const parsed = decodeRuntimeEvent({
      type: "content.delta",
      eventId: "event-pi-control-1",
      provider: "pi",
      providerInstanceId: "pi",
      createdAt: "2026-02-28T00:00:00.000Z",
      threadId: "thread-1",
      ...
      raw: {
        source: "pi.rpc.control",
        method: "command_output",
        payload: { type: "command_output", text: "ok" },
      },
    });

    expect(parsed.raw?.source).toBe("pi.rpc.control");
  });
