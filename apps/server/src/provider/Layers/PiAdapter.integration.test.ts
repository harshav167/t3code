...
import type { RpcCommand } from "../pi/PiModels.ts";
import type { PiRpcTransport } from "../pi/PiRpcTransport.ts";
...
it.layer(HarnessLayer)("PiAdapter integration", (it) => {
  it.effect("starts a session, streams assistant text, and completes the turn", () =>
    Effect.gen(function* () {
      ...
      yield* fake.pushEvent({ type: "agent_end", messages: [] } as AgentSessionEvent);
