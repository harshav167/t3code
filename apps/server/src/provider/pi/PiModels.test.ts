...
describe("piModelCapabilities", () => {
  ...
  it.each(["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"])(
    "uses discovered %s thinking metadata",
    (id) => {
      ...
      const thinking = (capabilities.optionDescriptors ?? []).find(
        (descriptor) => descriptor.id === "thinking",
      );

      expect(thinking?.type === "select" ? thinking.options.map(({ id }) => id) : []).toEqual([
        "off",
        "low",
        "medium",
        "high",
...
describe("extractAvailableModels", () => {
  ...
  it("keeps valid models while dropping malformed entries", () => {
    expect(
      extractAvailableModels(
        asResponse({
          type: "response",
          success: true,
          data: {
            models: [null, "bad", { provider: "openai" }, { provider: "openai", id: "gpt-5" }],
          },
        }),
      ),
