...
it.effect("waits for the terminal agent_end after an automatic retry", () =>
  withFakePi(
    {
      PI_MOCK_RETRY_ONCE: "1",
      PI_MOCK_ASSISTANT_TEXT: '{"title":"Final retry result"}',
    },
    (textGeneration) =>
      Effect.gen(function* () {
        const result = yield* textGeneration.generateThreadTitle({
        ...
        expect(result.title).toBe("Final retry result");
      }),
  ).pipe(Effect.provide(PiTextGenerationTestLayer)),
);
