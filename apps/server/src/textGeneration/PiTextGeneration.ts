...
} from "../provider/pi/PiModels.ts";
import { makePiRpcTransport } from "../provider/pi/PiRpcTransport.ts";
...
export const makePiTextGeneration = Effect.fn("makePiTextGeneration")(function* (
  ...
  const runPiPrompt = (input: {
    ...
    Effect.gen(function* () {
      ...
      const prompt = yield* transport.request(
        { type: "prompt", message: input.message },
        "pi-textgen-prompt",
        PI_LAST_TEXT_TIMEOUT_MS,
      );
      if (!piResponseSucceeded(prompt, "prompt")) {
        return yield* new TextGenerationError({
          operation: input.operation,
          detail: "The Pi runtime did not accept the text-generation prompt.",
        });
      }
